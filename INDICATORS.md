...
| HOT-3 | Agentive consumer — higher-order states guide belief formation and action | ◑ | `IdentityDocument.apply_amendment()` updates self-concept from reflections; mood drift (issue #62: now reverts to baseline rather than monotonically decaying) modulates generation temperature indirectly | Amendments are additive text appends, not structured belief updates; no downstream action |
