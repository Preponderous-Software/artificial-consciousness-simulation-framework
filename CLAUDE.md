...
│   ├── mood_semantics.py    # SemanticMoodScorer — optional embedding-similarity
│   │                        #   mood trigger strengths against per-dimension
│   │                        #   anchor phrases; off by default (#21)
...
- `IdentityDocument.drift_mood()` takes optional per-dimension `trigger_strengths` in `[0.0, 1.0]` (#21). `None` (the default and the shipped config's behaviour) runs the lexical `_MOOD_TRIGGERS` substring match, where every hit contributes a strength of exactly 1.0 — the pre-#21 semantics. When strengths are supplied, `text` is not inspected at all, values outside `[0, 1]` are clamped, and the dimension set is the union of the supplied keys with those already in `mood`, so an unanchored dimension still gets its homeostatic reversion instead of freezing. Because strength scales only the *drift* term, the equilibrium `baseline + strength * drift_rate / homeostasis_rate` is bounded above by the all-lexical-hits equilibrium — enabling semantic scoring cannot make a #134-style saturation worse.
- `mood.semantic` (optional, `enabled: false` in the shipped config) builds a `core.mood_semantics.SemanticMoodScorer` on the *same* provider the rest of the run uses, so it inherits the embed cache (#113) and circuit breaker (#114) (#21). Anchor phrases are embedded once per process, guarded by an `asyncio.Lock` with a double-check so overlapping callers cannot embed them twice; a failed anchor pass is not memoised and is retried on the next cycle. `Consciousness._score_mood_triggers()` catches an embed failure, logs a `WARNING`, and returns `None` so the cycle falls back to lexical triggers — this is *not* a #46-style silent fallback, since nothing fabricates model output and the thought itself was already generated.
